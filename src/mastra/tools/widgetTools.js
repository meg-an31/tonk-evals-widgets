...
import { z } from 'zod';
// TODO: rip out validation functions from in here for deterministic checks
export async function createWidgetTools(mcpServer) {
    return {
...
export default widgetDefinition;`;
                    const indexPath = `${widgetDir}/index.js`;
                    await mcpServer.executeTool('write_widget_file', {
                        path: indexPath,
        ...
        // Tool to read development documentation
        readDocumentation: createTool({
            id: 'read_documentation',
            description: 'Read development documentation for insights on how to build widgets',
            inputSchema: z.object({
            ...
            execute: async ({ context }) => {
                const { templateType } = context;
                try {
                    const templateResult = await mcpServer.executeTool('read_documentation', {
                        templateType
                    });
                    const templateData = JSON.parse(templateResult.content[0].text);
                    return {
                        ...
                        templates: templateData.templates,
                        guidelines: templateData.guidelines,
                        message: `Successfully read ${templateType} docs`
                    };
                }
                catch (error) {
                    return {
